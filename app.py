...
def plan_je_dag_tab(reizen_df, restaurants_df):
    ...
    if gekozen_locatie:
        ...
        # Stap 2: selecteer één restaurant per maaltijd
        ...
        # Recap zonder punten, alleen tekst
        st.markdown("## Jouw dagplanning:")
        st.markdown(f"### Bestemming: {gekozen_locatie}")

        st.markdown(f"Ontbijt: {ontbijt_keuze if ontbijt_keuze != '- geen -' else 'geen geselecteerd'}")
        st.markdown(f"Lunch: {lunch_keuze if lunch_keuze != '- geen -' else 'geen geselecteerd'}")
        st.markdown(f"Diner: {diner_keuze if diner_keuze != '- geen -' else 'geen geselecteerd'}")
